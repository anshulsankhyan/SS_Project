...
bool addAccount(int connFd) {
    ...
    int temp = addCustomer(connFd, true, newAccount.accountNumber);
    if (temp == -1) {
        return false;
    }
    newAccount.owners[0] = temp;

    if (newAccount.isAccountRegular)
        newAccount.owners[1] = -1;
    else {
        int tmp = addCustomer(connFd, false, newAccount.accountNumber);
        if (tmp == -1)
            return false;
        newAccount.owners[1] = tmp;
    } 
...
int addCustomer(int connFD, bool isPrimary, int newAccountNumber) {
    ...
    if (readBytes == -1) {
        ...
        for (int i = 0; i < strlen(tempBuffer); i++) {
            ...
            if (!((ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z'))) {
                sprintf(writeBuffer, "Invalid Input!");
    ...
    if (readBuffer[0] == 'M' || readBuffer[0] == 'F' || readBuffer[0] == 'O'|| readBuffer[0] == 'm'|| readBuffer[0] == 'f')
    ...
    if (customerAge <= 0 || customerAge < 5 || customerAge > 100) {
        // Either client has sent age as 0 (which is invalid) or has entered a non-numeric string
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer, "It seems you have passed a sequence of alphabets when a number was expected or you have entered an invalid age!\n\nYou'll now be redirected to the main menu!^");
        writeBytes = write(connFD, writeBuffer, strlen(writeBuffer));
        readBytes = read(connFD, readBuffer, sizeof(readBuffer)); // Dummy read
        return -1;
    }
...
bool modifyCustomerInfo(int connFD) {
    ...
    case 2:
        ...
        if (updatedAge <= 0 || updatedAge < 5 | updatedAge> 100) {
            // Either client has sent age as 0 (which is invalid) or has entered a non-numeric string
            bzero(writeBuffer, sizeof(writeBuffer));
            strcpy(writeBuffer, "It seems you have passed a sequence of alphabets when a number was expected or you have entered an invalid number!\nYou'll now be redirected to the main menu!^");
            writeBytes = write(connFD, writeBuffer, strlen(writeBuffer));
